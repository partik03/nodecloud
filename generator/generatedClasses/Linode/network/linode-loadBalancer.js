...
class Linode_LoadBalancer {
    ...
    * @param {AnyKeyword} filters - Data required for getNodeBalancers
    ...
    /**
        * Trigers the getNodeBalancer function of LoadBalancerLinodeClass
        * @param {NumberKeyword} nodeBalancerId - Data required for getNodeBalancer
        * @returns {Promise<getNodeBalancerResponse>}
        */
    describe(nodeBalancerId) {
        return new Promise((resolve, reject) => {
            this._linode.getNodeBalancer(nodeBalancerId)
                .then(data => resolve(data))
                .catch(err => reject(err));
        });
    }
